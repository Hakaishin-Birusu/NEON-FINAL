...
interface TokenWarningCardProps
  extends PropsOfExcluding<typeof Wrapper, "error"> {
  token?: Token;
}
...
export function TokenWarningCards({
  ...
  const { chainId } = useActiveWeb3React();
  const [dismissedToken0, dismissToken0] = useTokenWarningDismissal(
    chainId,
    currencies[Field.INPUT]
  );
  const [dismissedToken1, dismissToken1] = useTokenWarningDismissal(
    chainId,
    ...
    <WarningContainer className="token-warning-container">
      <AutoColumn gap="lg">
        ...
        <TYPE.body color={"red2"}>
          ...
          addresses. Please do your own research before interacting with any
          ERC20 token.
        </TYPE.body>
        {Object.keys(currencies).map((field) => {
          const dismissed =
            field === Field.INPUT ? dismissedToken0 : dismissedToken1;
